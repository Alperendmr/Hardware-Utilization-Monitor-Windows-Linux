...
from appui1 import Ui_MainWindow
import psutil
import threading
import time
import sys
        ...
        try:
            while True:
                ...
                self.ram_info.setText("\nAvailable Memory : " + str(
                    round(psutil.virtual_memory().available / (1024.0 ** 3),
                          2)) + " GB" + "\n\n" + "Used Memory : " + str(
                    round(psutil.virtual_memory().used / (1024.0 ** 3), 2)) + " GB")
                self.diskinfo.setText(
                    "\nTotal Disk : " + str(round(obj_Disk.total / (1024.0 ** 3), 2)) + " GB\n\nUsed Disk : " + str(
                        round(obj_Disk.used / (1024.0 ** 3), 2)) + " GB\n\nFree Disk : " + str(
                        round(obj_Disk.free / (1024.0 ** 3), 2)) + " GB")
                time.sleep(1)
        except:
